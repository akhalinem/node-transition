...
class ResourceManager extends EventEmitter {
  ...
  async connectAll() {
    await Promise.all(
      this.resources.entries().map(async ([name, resource]) => {
        console.log(`Connecting to resource: ${name}`);
        await resource.connect();
        console.log(`Resource connected: ${name}`);
  ...
  async shutdown(signal) {
    ...
    // Allow the shutdown timeout to be ignored by the event loop
    this.shutdownTimeout.unref();

    while (this.resources.size > 0) {
      const [name, resource] = Array.from(this.resources).pop();
      try {
        console.log(`Shutting down resource: ${name}`);
        await resource.close();
        this.resources.delete(name);
      } catch (e) {
        console.error(`Error shutting down resource ${name}:`, e);
      }
    }

    console.log("All resources have been shut down gracefully.");
...
setTimeout(() => {
  ...
  class GracefulHTTPServer {
    ...
    async shutdown(signal) {
      ...
      this.isShuttingDown = true;
      console.log(`\nReceived shutdown signal: ${signal}`);
      console.log("Starting HTTP server graceful shutdown...");
      ...
      while (this.activeRequests > 0) {
        ...
        if (elapsed >= this.shutdownTimeout) {
          console.log(
            "Shutdown timeout reached, force-closing active connections"
          );
          for (const socket of this.connections) {
            if (!socket.destroyed) {
              socket.destroy();
      ...
      // Shutdown resources if ResourceManager is provided
      if (this.resourceManager) {
        await this.resourceManager.shutdown(signal);
      }

      console.log("HTTP server has been shut down gracefully.");
      process.exit(0);
    }
  }
