...
setTimeout(() => {
  ...
  async function fetchAllAPIs(apiConfigs, timeoutMs) {
    const wrappedPromises = apiConfigs.map((apiCall) =>
      fetchWithTimeout(apiCall, timeoutMs)
    );
    ...
    results.forEach((result, index) => {
      if (result.status === "fulfilled") {
        ...
        if (result.reason.message === "Timeout") {
          timeouts.push({ api: index + 1, error: result.reason.message });
        } else {
          failures.push({ api: index + 1, error: result.reason.message });
        }
      }
    });
...
setTimeout(() => {
  ...
  async function testChallenge3() {
    ...
    try {
      const result = await retryWithBackoff(flaky, 5, 1000);
      console.log("\n✓ Result:", result);
